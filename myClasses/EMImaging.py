...
class dataProcessing(object):
	##############################################################
	def __init__(self,inputDir,inputFile,magnification,numPixels=1024,fps=10,microscopeName='JOEL2010'):
		"""
		Initializes inputDir, inputFile, magnification, numPixels, and microscopeName. Also calls
		self.countFrame() and initializes row, col, and numFrames.
		Usage           --- Gets called automatically during object creation
		Input arguments --- NULL
		Returns         --- NULL
		Initializes     --- self.inputDir, self.inputFile,
							self.magnification, self.numPixels, self.microscopeName,
							self.row, self.col, self.numFrames
		"""
		print 'INITIALIZING A FEW VARIABLES'
		self.inputDir = inputDir
		self.inputFile = inputFile
		self.magnification = magnification
		self.numPixels = numPixels
		self.fps = fps
		self.microscopeName = microscopeName
		#self.countFrames()
		[self.pixInNM,self.pixInAngstrom] = myPythonFunc.findPixelWidth(mag=magnification,numPixels=numPixels,microscopeName=microscopeName)

		self.metaData={}
		self.metaData['inputDir'] = self.inputDir
		#metaData['inputFile'] = self.inputFile
		self.metaData['magnification'] = self.magnification
		self.metaData['numPixels'] = self.numPixels
		self.metaData['fps'] = self.fps
		self.metaData['microscopeName'] = self.microscopeName
		#metaData['row']=self.row
		#metaData['col']=self.col
		#metaData['numFrames']=self.numFrames
		#metaData['frameList']=range(1,self.numFrames+1)
		self.metaData['pixInNM']=self.pixInNM
		self.metaData['pixInAngstrom']=self.pixInAngstrom
	...
	##############################################################
	def readTIFF(self, inputFile):
		self.inputFile = inputFile
		print "CREATING THE RAW IMAGE NUMPY STACK"
		myPythonFunc.mkdir(self.inputDir+'/output/data/dataProcessing/gImgRawStack')
		myPythonFunc.mkdir(self.inputDir+'/output/images/dataProcessing/gImgRawStack')
		self.numFrames = 0
		stack = TIFF.open(self.inputFile, mode='r')
		for gImgRaw in stack.iter_images():
			...
			self.numFrames+=1
			numpy.save(self.inputDir+'/output/data/dataProcessing/gImgRawStack/'+str(self.numFrames)+'.npy', gImgRaw)
			cv2.imwrite(self.inputDir+'/output/images/dataProcessing/gImgRawStack/'+str(self.numFrames)+'.png', gImgRaw)
		self.metaData['row']=self.row
		self.metaData['col']=self.col
		self.metaData['numFrames']=self.numFrames
		self.metaData['frameList']=range(1,self.numFrames+1)
		pickle.dump(self.metaData, open(self.inputDir+'/metaData', 'wb'))
	##############################################################
	...
	#def readAVI(self, inputFile):
		...
		#myPythonFunc.mkdir(self.inputDir+'/output/data/dataProcessing/gImgRawStack')
		#myPythonFunc.mkdir(self.inputDir+'/output/images/dataProcessing/gImgRawStack')
		#stack = skvideo.io.VideoCapture(self.inputFile)
		#if (stack.isOpened() == True):
			#self.row, self.col, self.numFrames = stack.info['streams'][0]['height'], stack.info['streams'][0]['width'], stack.info['streams'][0]['nb_frames']
			#self.frameList=range(1,self.numFrames+1)
			#for frame in self.frameList:
				#flag, gImg = stack.read()
				#if (len(gImg.shape) == 3):
					#gImg = gImg[:,:,0]
	...
	def countFrames(self):
		"""
		Counts the number of frames, and size of the inputFile.
		Usage           --- self.countFrames()
		Input arguments --- NULL
		Returns         --- NULL
		Initializes     --- self.row, self.col, self.numFrames
		"""
		print "COUNTING THE NUMBER OF FRAMES IN THE IMAGE STACK"
		self.numFrames = 0
		stack = cv2.VideoCapture(self.inputFile)
		if (stack.isOpened() == True):
			while(1):
		...
		if (method == 'mean'):
			...
			for i in range(partitions[0]):
				for j in range(partitions[1]):
					gImgROI = gImg[subRow[i,0]:subRow[i,1], subCol[j,0]:subCol[j,1]]
					if (excludeZero == True):
						threshold = skimage.filters.threshold_otsu(gImgROI[gImgROI>0])
					else:
						threshold = skimage.filters.threshold_otsu(gImgROI)
					bImgROI = gImgROI>=threshold
					bImg[subRow[i,0]:subRow[i,1], subCol[j,0]:subCol[j,1]] = bImgROI
		elif (method == 'custom'):
