...
def angAvg2D_py(fImg, qArr):
	qMax = qArr.max()+1
	avg = numpy.zeros(qMax)
	avg2 = numpy.zeros(qMax)
	counter = numpy.zeros(qMax)

	ringVals = {i:[] for i in range(qMax)}

	for f,q in zip(fImg.flat, qArr.flat):
		avg[q] += f
		avg2[q] += f*f
		counter[q] += 1
#		ringVals[qArr[f,q]].append(fImg[f,q])
	...
	var = avg2 - avg*avg
	measure = numpy.sqrt(var)/(avg*(avg>0.)+(avg<=0.))

	return (avg, measure, ringVals)


def filterFourier_py(fImg, qArr):
	# TODO: Test and debug code.
	# Input: complex Fourier transform
	# Output: complex filtered Fourier transform
	#
	# Find avg and stdDev of Fourier amplitudes along resolution rings
	# Revisit resolultion rings and set to zero all complex Fourier components that are deviant.
	# Send this output back to Python.
	avg = numpy.zeros(qArr.max()+1)
	avg2 = numpy.zeros(qArr.max()+1)
	counter = numpy.zeros(qArr.max()+1, dtype=int)
	[r, c] = qArr.shape
	output = numpy.zeros([r,c], dtype='uint8')
#	output = numpy.zeros([r,c], dtype='complex128')
	...
	for i in range(r):
		for j in range(c):
			avg[qArr[i,j]] += numpy.abs(fImg[i,j])
			avg2[qArr[i,j]] += numpy.abs(fImg[i,j] * fImg[i,j])
			counter[qArr[i,j]] += 1
	...
	for i in range(r):
		for j in range(c):
			k = qArr[i,j]
			temp = abs(fImg[i,j])
			if (abs(temp-avg[k]) > (cutoff*avg2[k])) and (avg[k] > 0):
				# output[i,j] = fImg[i,j]
				output[i,j] = 1
	return output
...
def plotOrderParam(outputDir, ordParamArr, gImg, pixInAngstrom, gamma=0.4, ringRange = [0, 10000], step_size=10, manualXTicks=[], fps=10):
	...
	#temp0 = [abs(x - ringRange[0]) for x in qTcks]
	#temp1 = [abs(x - ringRange[1]) for x in qTcks]
	end = temp0.index(min(temp0))
	start = temp1.index(min(temp1))
	
	#start = temp0.index(min(temp0))
	#end = temp1.index(min(temp1))
	
	print end-start
	fig = plt.figure(figsize=(3.5,3.5))
	ax = fig.add_axes([0,0,1,1])
	#ax.imshow(ordParamArr[:,start:end+1]**gamma, cmap='jet', aspect='equal', origin='lower', extent=[0,end-start,1.0/fps,1.0*row/fps])
	...
	fig = plt.figure(figsize=(3.5,3.5))
	ax = fig.add_axes([0,0,1,1])
	#ax.imshow(ordParamArr[:,start:end+1]**gamma, cmap='jet', aspect='equal', origin='lower', extent=[qTcks[start],qTcks[end],1.0/fps,1.0*row/fps])
